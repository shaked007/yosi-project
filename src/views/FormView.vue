<template>
  ...
  <div class="flex-and-submit-container">
    <form
      ref="report-form"
      method="POST"
      @submit.once="handleSubmit($event)"
      :action="currentUrl"
    >
        ...
        <div class="flex-inputs">
          ...
          <div id="date">
            <q-input
              ...
              ref="timeRef"
              name="tidruh-time"
              placeholder=" שעת התדרוך  (לחץ על השעון)"
          ...
          <div>
            <div class="nosim-container">
              <v-btn @click="handleMinus" class="ma-2" color="dark" dark>
                <v-icon dark right> mdi-minus-thick </v-icon>
              </v-btn>

              <span class="nosim-title"> נוסעים ({{ nosim }})</span>
              <v-btn @click="handlePlus" class="ma-2" color="dark" dark>
          ...
          <div id="radios">
            <v-radio-group
              ...
              v-model="radio1"
              class="font-sizer"
              label="האם הנהג ישן 7 שעות לפני ביצוע המשימה?"
            >
            ...
            <v-radio-group
              ...
              ref="radio2"
              class="font-sizer"
              label="האם לנהג יש רישיון והיתר מתאים לסוג הרכב?"
            >
            ...
            <v-radio-group
              ...
              v-model="radio3"
              ref="radio3"
              label="האם הנהג נסע בציר זה בעבר?"
            >
              <v-radio
                label="כן"
                name="is-drove-before"
                :value="true"
              ></v-radio>
              <v-radio
                label="לא"
          ...
          <div>
            <div class="nosim-container">
              <v-btn @click="handleMinusStops" class="ma-2" color="dark" dark>
                <v-icon dark right> mdi-minus-thick </v-icon>
              </v-btn>
              <span class="nosim-title"> יעדי עצירה</span
              ><span class="nosim-title" v-if="stops"> ({{ stops }})</span>

              <v-btn @click="handlePlusStops" class="ma-2" color="dark" dark>
                <v-icon dark right> mdi-plus-thick </v-icon>
              </v-btn>
            </div>
            <span class="nosim-title" v-if="!stops">
              לא נרשמו יעדי עצירה בנסיעה זו</span
            >
          ...
          <div>
            <div class="nosim-container">
              <v-btn @click="handleMinusPhones" class="ma-2" color="dark" dark>
                <v-icon dark right> mdi-minus-thick </v-icon>
              </v-btn>

              <span class="nosim-title"> אנשי קשר חיוניים ({{ phones }})</span>
              <v-btn @click="handlePlusPhones" class="ma-2" color="dark" dark>
                <v-icon dark right> mdi-plus-thick </v-icon>
              </v-btn>
            </div>
          ...
          <v-checkbox required name="is-tudrah" label="הנהג עבר תדרוך נסיעה">
          </v-checkbox>
      ...
      <div class="phone-container">
        <h6>גדוד 383 - מ"פ מפקדה</h6>
        <h6><a href="tel:054-993-6883"> 0529200800</a> - מעיין כפיר</h6>
      </div>
      <br />
      <input
...
export default {
  data() {
    return {
      ...
      finishedTime: "",
      cars: [
        "אופל קומבו - צ`158-641",
        "אופל קומבו צ`158-632",
        "אופל קומבו צ`158-637",
        "אופל קומבו צ`158-847",
        "שברולט סוואנה - צ`297-616",
        "שברולט קולורדו - צ`187-088",
        "שברולט קולורדו - צ`187-099",
        // "גרור - צ`298-753",
        'מג"ד - קיה נירו - 353-56-803',
        'סמג"ד - קיה ריו - 167-54-303',
        'מ"פ טכנ"ל - קיה ריו - 176-64-003',
        'יונדאי - 359-86-602 I20 - מ"פ הפת"ק',
        'מ"פ תקשורת - סיאט איביזה - 458-32-902',
        'מ"פ שו"ב - קיה ריו - 274-26-203',
        'רס"ר - מאזדה 2 - 736-34-903',
        "ממסרים שכור - קיה פיקנטו - 254-10-902",
        'מ"פ מפקדה - סיאט איביזה - 604-10-202',
        'רת"ח אתרים - סיאט איביזה - 602-74-002',
      ],
      // squadCommanderCar: [],
      ...
      generatedLink: "",
      jobs: [
        { hebrewName: "מפקד משלח משימה", name: "mefaked-meshaleh-mesima" },
  ...
  mounted() {
    this.$swal.fire({
      icon: "warning",
      title: 'ב"ם',
      text: 'אין לפרט על משימות מסווגות ו/או לפרט שמות מתקנים מסווגים, יש לשמור על ב"מ',
      timer: 4000,
    });
  },
  methods: {
