<template>
  ...
  <div class="filter-container" v-if="filterType == 'driver'">
    ...
    <q-input
      ...
      :model-value="
        !dateModel.from
          ? ''
          : `${`${dateModel.to.split('/')[2]}/${dateModel.to.split('/')[1]}/${
              dateModel.to.split('/')[0]
            }`} - ${`${dateModel.from.split('/')[2]}/${
              dateModel.from.split('/')[1]
            }/${dateModel.from.split('/')[0]}`}`
      "
      readonly
      ref="timeRef"
      name="tidruh-time"
    ...
    <button v-if="driverModel" @click="filterAndNavigate">
      <v-icon> mdi mdi-printer</v-icon> הדפס
    </button>
  </div>
  <div class="filter-container" v-if="filterType == 'car'">
    ...
    <button v-if="carValue" @click="filterAndNavigate">
      <v-icon> mdi mdi-printer</v-icon> הדפס
    </button>
  ...
  methods: {
    ...
    async filterAndNavigate() {
      if (this.filterType == "car") {
        ...
        if (this.dateModel.from && this.dateModel.to) {
          this.dates = `${this.dateModel.to.split("/")[2]}/${
            this.dateModel.to.split("/")[1]
          }/${this.dateModel.to.split("/")[0]} - ${
            this.dateModel.from.split("/")[2]
          }/${this.dateModel.from.split("/")[1]}/${
            this.dateModel.from.split("/")[0]
          } `;
        } else {
    ...
    handleRangeAndDriverFilter(object) {
      ...
      if (object.from && object.to) {
        ...
        return [
          ...this.drivesData.filter((car) => {
            let stru = car.date.split(" ")[car.date.split(" ").length - 1];
            stru = new Date(
              `${stru.split("/")[1]}/${stru.split("/")[0]}/${
                stru.split("/")[2]
              }`
            );
            return (
              this.dateCheck(this.startingDate, this.endingDate, stru) &&
              car.driver.includes(this.driverModel)
            );
          }),
        ];
      }
      return [
        ...this.drivesData.filter((driver) => {
          return driver.driver.includes(this.driverModel);
        }),
      ];
    },
    handleRangeAndCarFilter(object) {
      ...
      if (object.from && object.to) {
        ...
        return [
          ...this.drivesSortedObject[this.carValue].filter((car) => {
            let stru = car.date.split(" ")[car.date.split(" ").length - 1];
            stru = new Date(
              `${stru.split("/")[1]}/${stru.split("/")[0]}/${
                stru.split("/")[2]
