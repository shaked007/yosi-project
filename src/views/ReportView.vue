<template>
  <h3 class="no-permissions-header" v-if="!isAuthenticated">
  ...
  <button
    ...
    to="/print-view"
  >
    <v-icon x-large size="60"> mdi mdi-printer</v-icon>
  </button>
        ...
        <div class="flex-inputs">
          ...
          <div>
            ...
            <q-input
              ...
              readonly
              required
              :value="report['tidruh-time']"
              name="tidruh-time"
              :placeholder="report['tidruh-time']"
            >
          ...
          <div>
            ...
            <div
              ...
              v-for="(nosea, index) in Object.keys(drivesObject)"
              :key="nosea"
            >
          ...
          <v-radio-group
            readonly
            required
            v-model="report['is-drove-before']"
          ...
          <div>
            <h6>יעדי עצירה</h6>
            <h6 v-if="!Object.keys(stopsObject).length">
              לא נרשמו יעדי עצירה בנסיעה זו
            </h6>

            <div
              class="names-container"
              v-for="(stop, index) in Object.keys(stopsObject)"
              :key="stop"
            >
          ...
          <div>
            ...
            <div
              class="names-container"
              v-for="(phone, index) in Object.keys(contactsObject)"
              :key="phone"
            >
...
export default {
  ...
  data() {
    return {
      ...
      url:
        "/.netlify/functions/fetch_chosen?id=" +
  ...
  methods: {
    ...
    async getReportById() {
      ...
      this.currentDate = moment(
        new Date(this.report["date"]),
        "L",
        "he"
      ).format("יום dddd  D/M/y");

      Object.keys(this.report).forEach((item) => {
        if (item.includes("nosea")) {
          if (this.drivesObject[item.split("nosea")[1][0]]) {
            if (item.includes("num")) {
              this.drivesObject[item.split("nosea")[1][0]]["num"] =
                this.report[item];
            } else {
              this.drivesObject[item.split("nosea")[1][0]]["name"] =
                this.report[item];
            }
          } else {
        ...
        if (item.includes("contact")) {
          if (this.contactsObject[item.split("contact")[1][0]]) {
            if (item.includes("num")) {
              this.contactsObject[item.split("contact")[1][0]]["num"] =
                this.report[item];
            } else {
              this.contactsObject[item.split("contact")[1][0]]["name"] =
                this.report[item];
            }
          } else {
        ...
        if (item.includes("stop")) {
          if (this.stopsObject[item.split("stop")[1][0]]) {
            if (item.includes("num")) {
              this.stopsObject[item.split("stop")[1][0]]["num"] =
                this.report[item];
            } else {
              this.stopsObject[item.split("stop")[1][0]]["name"] =
                this.report[item];
            }
          } else {
      ...
      this.report.date = moment(new Date(this.report.date), "L", "he").format(
        "יום dddd  D/M/y"
      );
