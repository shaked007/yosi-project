<template>
  ...
  <div class="password-container" v-if="!isAuthenticated">
    <v-text-field
      type="password"
      :error="isError"
      @input="checkIfZeroLength"
      :placeholder="isError ? 'סיסמא לא נכונה' : 'הכנס סיסמא'"
      v-model="passValue"
    >
    </v-text-field>
    <button class="check-password-button" @click="checkPassword">
      בדוק סיסמא
    </button>
  </div>
  <div class="text-center">
    <v-progress-circular
      v-if="!isFinished && spinnerStarter"
      indeterminate
      color="white"
      size="60"
    ></v-progress-circular>
    ...
    <v-carousel height="80vh" light hide-delimiters v-model="slide">
      <v-carousel-item :value="car.carName" v-for="car in cars" :key="car.key">
        ...
        <div class="filters-container" v-if="isFiltering">
          <q-input
            ...
            :model-value="
              !car.dateModel.from
                ? ''
                : `${`${car.dateModel.to.split('/')[2]}/${
                    car.dateModel.to.split('/')[1]
                  }/${car.dateModel.to.split('/')[0]}`} - ${`${
                    car.dateModel.from.split('/')[2]
                  }/${car.dateModel.from.split('/')[1]}/${
                    car.dateModel.from.split('/')[0]
                  }`}`
            "
            readonly
            ref="timeRef"
        ...
        <div class="btn-filters-container" v-if="isFiltering">
          <button class="revert-filter-btn sanen-btn" @click="handleAll(car)">
            סנן
          </button>
          <button class="revert-filter-btn" @click="revertFilter(car)">
            בטל סינון
          </button>
        </div>
        <table>
          ...
          <tbody>
            <tr
              v-for="(drive, index) in drivesSortedObject[car.carName]"
              :key="drive['_id']"
            >
              <td>
                <button
                  ...
                  @click="
                    deleteRecord(
                      drive,
                      index,
                      drivesSortedObject[car.carName],
                      car
                    )
                  "
                >
...
export default {
  data() {
    return {
      ...
      cars: [
        {
          key: "אופל קומבו - צ`158-641",
          carName: "אופל קומבו - צ`158-641",
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: "אופל קומבו צ`158-632",
          carName: "אופל קומבו צ`158-632",
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: "אופל קומבו צ`158-637",
          carName: "אופל קומבו צ`158-637",
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: "אופל קומבו צ`158-847",
          carName: "אופל קומבו צ`158-847",
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: "שברולט סוואנה - צ`297-616",
          carName: "שברולט סוואנה - צ`297-616",
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: "שברולט קולורדו - צ`187-088",
          carName: "שברולט קולורדו - צ`187-088",
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: "שברולט קולורדו - צ`187-099",
          carName: "שברולט קולורדו - צ`187-099",
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        // {
        //   key: "גרור - צ`298-753",
        //   carName: "גרור - צ`298-753",
        //   driverModel: "",
        //   dateModel: { from: "", to: "" },
        // },
        {
          key: 'מג"ד - קיה נירו - 353-56-803',
          carName: 'מג"ד - קיה נירו - 353-56-803',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: 'סמג"ד - קיה ריו - 167-54-303',
          carName: 'סמג"ד - קיה ריו - 167-54-303',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: 'מ"פ טכנ"ל - קיה ריו - 176-64-003',
          carName: 'מ"פ טכנ"ל - קיה ריו - 176-64-003',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: 'יונדאי - 359-86-602 I20 - מ"פ הפת"ק',
          carName: 'יונדאי - 359-86-602 I20 - מ"פ הפת"ק',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: 'מ"פ תקשורת - סיאט איביזה - 458-32-902',
          carName: 'מ"פ תקשורת - סיאט איביזה - 458-32-902',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: 'מ"פ שו"ב - קיה ריו - 274-26-203',
          carName: 'מ"פ שו"ב - קיה ריו - 274-26-203',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: 'רס"ר - מאזדה 2 - 736-34-903',
          carName: 'רס"ר - מאזדה 2 - 736-34-903',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: "ממסרים שכור - קיה פיקנטו - 254-10-902",
          carName: "ממסרים שכור - קיה פיקנטו - 254-10-902",
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: 'מ"פ מפקדה - סיאט איביזה - 604-10-202',
          carName: 'מ"פ מפקדה - סיאט איביזה - 604-10-202',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
        {
          key: 'רת"ח אתרים - סיאט איביזה - 602-74-002',
          carName: 'רת"ח אתרים - סיאט איביזה - 602-74-002',
          driverModel: "",
          dateModel: { from: "", to: "" },
        },
      ],
  ...
  methods: {
    handleAll(car) {
      this.drivesSortedObject[car.carName] = [
        ...this.backupReports[car.carName],
      ];
    ...
    async deleteRecord(drive, index, arr, car) {
      const swalAction = await this.$swal({
        icon: "warning",
        showCancelButton: true,
        showConfirmButton: true,
        confirmButtonText: "מחק",
        cancelButtonText: "חזור",
        title: ":האם את בטוח שאתה רוצה למחוק את ",
        text: `דוח שבוצע בתאריך ${drive.date} ברכב ${this.slide}`,
        width: "300px",
        height: "300px",
      });
    ...
    handleDriver(val) {
      console.log(val);
      if (val.trim()) {
        this.drivesSortedObject[this.slide] = this.drivesSortedObject[
          this.slide
        ].filter((o) => {
    ...
    async getDrivesData() {
      ...
      this.drivesData.forEach((drive) => {
        drive.date = moment(new Date(drive.date), "L", "he").format(
          "יום dddd  D/M/y"
        );
    ...
    handleRange(object) {
      ...
      if (object.from && object.to) {
        ...
        this.drivesSortedObject[this.slide] = this.drivesSortedObject[
          ...
          stru = new Date(
            `${stru.split("/")[1]}/${stru.split("/")[0]}/${stru.split("/")[2]}`
          );
